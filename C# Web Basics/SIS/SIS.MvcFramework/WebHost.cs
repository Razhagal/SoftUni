...
namespace SIS.MvcFramework
{
    public static class WebHost
    {
        ...
        public static void AutoRegisterRoutes(IMvcApplication application, IServerRoutingTable serverRoutingTable,
        ...
        private static IHttpResponse ProcessRequest(DependencyContainer.IServiceProvider serviceProvider, Type controllerType, MethodInfo action, IHttpRequest request)
        {
            ...
            var controllerIdentityUser = controllerInstance.User;
            ...
            if (authorizeAttribute != null && !authorizeAttribute.IsInAuthority(controllerIdentityUser))
            {
            ...
            foreach (var parameter in parameters)
            {
                    ...
                    && parameter.ParameterType != typeof(string))
                {
                    var collection = httpDataValue
                        .Select(x => Convert.ChangeType(x, parameter.ParameterType.GenericTypeArguments.First()));
            ...
            HttpResponse response = action.Invoke(controllerInstance, parameterValues.ToArray()) as HttpResponse;
            return response;
        }
